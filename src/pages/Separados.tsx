...
export default function Separados() {
  ...
  const handleProdKeyPress = async (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && prodSearch.trim() !== '') {
      ...
      // 1. Buscar coincidencia exacta por referencia (scanner de código de barras)
      let matched = items.find(p =>
        p.referencia && String(p.referencia).toLowerCase() === prodSearch.trim().toLowerCase()
      );
      // 2. Si no hay exacta por referencia, buscar por nombre
      if (!matched) {
      ...
      if (matched) {
        ...
        // 3. Fallback: consultar al backend por referencia exacta
        try {
          const res = await API.get(`/productos/buscar/${encodeURIComponent(prodSearch.trim())}`);
          if (res.data && res.data.id) {
            addProdToCart(res.data);
          }
        } catch {
          // Producto no encontrado en ninguna fuente
        }
      }
    }
  };
                  ...
                  filteredSeparados.map(s => {
                    ...
                    return (
                      <tr key={s.id} className="group hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => openView(s.id)}>
                        ...
                        <td className="px-8 py-6 text-center">
                          <span className={`px-4 py-1 rounded-full text-[9px] uppercase tracking-widest ${s.estado === 'Pendiente' ? 'bg-amber-100 text-amber-700' :
